...
) : Widget
{
    ...
    public FhirSection(
        ...
        titleAbbreviations, groupByResourceType)
    {
    }
    ...
    public FhirSection(
        ...
        : this(code,
            codedSectionBuilder != null
                ...
                : m_defaultBuilder, _ => severity, titleAbbreviations, groupByResourceType)
    {
    }
    ...
    )
    {
        ...
        if (sectionNav.EvaluateCondition("f:author"))
        {
            ...
            sectionContent.Add(new NameValuePair(
                [new LocalizedLabel("composition.section.author")],
                [
            ...
            sectionContent.Add(
                new AnyReferenceNamingWidget("f:focus",
                    widgetModel: new ReferenceNamingWidgetModel
                    {
                        ...
                        LabelOverride = new LocalizedLabel("composition.section.focus"),
                    }
                )
            );
        }
        ...
        if (entryNavs.Count != 0)
        {
            var groups = entryNavs
                .GroupBy(n => groupByResourceType ? n.Node?.Name : string.Empty).ToList();
            sectionContentType.Add(SectionElements.Entries);

            List<SectionBuildResult> sectionBuildResult = [];
            foreach (var group in groups)
            {
                sectionBuildResult.Add(await codedSectionBuilder(group.ToList(), group.Key, groups.Count > 1));
            }
        ...
        else if (sectionNav.EvaluateCondition("f:emptyReason"))
        {
            ...
            sectionContent.Add(
                new Widgets.Alert(
                    new NameValuePair(
                        [new LocalizedLabel("general.data-absent-reason")],
